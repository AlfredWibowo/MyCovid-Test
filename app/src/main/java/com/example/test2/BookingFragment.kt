        ...
        binding.btnNexttoSchedule.setOnClickListener {
            ...
            if (count == 0) {
                ...
                _actvTanggal.setOnClickListener {
                    ...
                    val _actvJam = dialog.findViewById<AutoCompleteTextView>(R.id.actvJamDialogSchedule)
                    val arrJam = listOf("17.30-18.00", "18.00-18.30", "19.00-19.30")
                    val dropdownAdapter = ArrayAdapter(requireContext(), R.layout.item_dropdown_dialog_schedule, arrJam)
                    _actvJam.setAdapter(dropdownAdapter)

                    //cek dialog
                    val _btnBookNow = dialog.findViewById<Button>(R.id.btnBookNowDialogSchedule)
                    _btnBookNow.setOnClickListener {
                        if (_actvJam.text.toString() == "dd/mm/YY" || _actvJam.text.toString() == "Select...") {
                            Toast.makeText(
                                requireContext(),
                                "check again!!!",
                                Toast.LENGTH_LONG
